...
import CopilotFunctionCallingProvider, {
  CopilotFunctionCallingOptions,
  FunctionCallResult,
} from '../providers/copilot-function-calling.provider';
...
export class ChatIntegrationService {
  ...
  /**
   * Send message with Copilot agent tools support
   */
  async sendMessageWithTools(
    ...
    Result<ChatMessage & { toolExecutions?: FunctionCallResult[] }, MLError>
  > {
    try {
      ...
      // Get Copilot provider with function calling
      const copilotProvider = this.providers.get(
        'github-copilot'
      ) as CopilotFunctionCallingProvider;
      if (
        !copilotProvider ||
        !(copilotProvider instanceof CopilotFunctionCallingProvider)
      ) {
        return {
          success: false,
          error: {
            code: 'PROVIDER_NOT_AVAILABLE',
            message: 'Copilot function calling provider not available',
            retryable: false,
          },
        };
      }
      ...
      const context = await this.buildAgentContext(session, options);

      // Generate response with tool support
      const response = await copilotProvider.generateTextWithTools(message, {
        enableFunctionCalling: options.enableTools !== false,
      ...
      const assistantMessage: ChatMessage = {
        ...
        metadata: {
          ...
          functionCalls: response.data.functionCalls?.length || 0,
          toolsUsed: response.data.functionCalls?.map(fc => fc.name) || [],
        },
      };
      ...
      if (!session.stats) {
        session.stats = { messagesCount: 0, toolsUsed: [] };
      }
      session.stats.messagesCount = (session.stats.messagesCount || 0) + 2;
      ...
      return {
        ...
        data: {
          ...assistantMessage,
          toolExecutions: response.data.functionCalls || [],
        },
      };
    } catch (error) {
      return {
  ...
  private async buildAgentContext(
    session: ChatSession,
    options: { agentType?: string; projectId?: string; workspaceId?: string }
    ...
    if (options.agentType) {
      const agentTools = AgentToolsRegistry.getToolsForAgent(options.agentType);
      context.availableTools = agentTools;
...
**Jak mogę Ci pomóc?**`;
  ...
  private exportToText(session: ChatSession, messages: ChatMessage[]): string {
    ...
    for (const message of messages) {
      lines.push('');
      lines.push(
        `[${message.timestamp.toISOString()}] ${message.role.toUpperCase()}${
