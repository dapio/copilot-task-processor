...
function useAgentsApi() {
      ...
      setAgents([
        {
          ...
          description: 'Requirements analysis and stakeholder management specialist',
  ...
  const updateAgentStatus = useCallback(async (agentId: string, isActive: boolean, currentWorkload?: number) => {
    try {
      const response = await fetch(`http://localhost:3003/api/agents/${agentId}/status`, {
        method: 'PUT',
...
function useProjectsApi() {
  ...
  const createProject = useCallback(async (projectData: { name: string; description: string; type: string }) => {
    try {
      const response = await fetch('http://localhost:3003/api/projects', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const updateProjectStatus = useCallback(async (projectId: string, status: string) => {
    try {
      const response = await fetch(`http://localhost:3003/api/projects/${projectId}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return { projects, loading, error, fetchProjects, createProject, updateProjectStatus };
}
    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-2">Overview of your AI-powered development platform</p>
        </div>
              ...
              agents.slice(0, 4).map((agent) => (
                <div key={agent.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-gray-900">{Math.round(agent.currentWorkload * 100)}%</div>
...
function ProjectsView() {
  const { projects, loading, error, fetchProjects, createProject, updateProjectStatus } = useProjectsApi();
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-2">Manage your development projects and teams</p>
        </div>
        <button 
...
function AgentsView() {
  const { agents, loading, error, fetchAgents, updateAgentStatus } = useAgentsApi();
...
function AgentCard({ agent, onUpdateStatus }: { 
  agent: Agent; 
  onUpdateStatus: (id: string, isActive: boolean, workload?: number) => Promise<any>; 
}) {
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="w-full h-2 bg-gray-200 rounded-full">
          <div 
            className={`h-2 rounded-full transition-all ${
              agent.currentWorkload > 0.8 ? 'bg-red-500' :
              agent.currentWorkload > 0.6 ? 'bg-yellow-500' : 
              'bg-green-500'
            }`}
            style={{ width: `${agent.currentWorkload * 100}%` }}
          />
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'agents':
        return <AgentsView />;
      case 'workflows':
        return <div className="p-8"><h1 className="text-2xl font-bold">Workflows (Coming Soon)</h1></div>;
      case 'communications':
        return <div className="p-8"><h1 className="text-2xl font-bold">Communications (Coming Soon)</h1></div>;
      case 'settings':
